...
from src.plugin_module import plugin_module, extract_dependency_matrix
...
class EmbeddingGAT(MessagePassing):
    ...
    def forward(self,
        ...
        alpha, alpha_raw = self.edge_updater(edge_index, alpha=alpha)
        ...
        if return_attention_weights:
            return out, (edge_index, alpha), alpha_raw
        else:
            return out
    ...
    def edge_update(self, alpha_j: Tensor,
        ...
        alpha_raw = alpha_j + alpha_i
        alpha = F.leaky_relu(alpha_raw, self.negative_slope)
...
class GDN(nn.Module):
    def __init__(self,
        ...
        self.out_layer = MLP(in_channels=hid_dim,
                             ...
                             act="relu",
                             norm="batch_norm")
        self.attn_ae = plugin_module(number_nodes, attn_hidden)
        self.loss_fun = nn.MSELoss(reduction='mean')
        self.loss_fn_attn = nn.MSELoss(reduction='mean')
        self.device = device
        self.reset_parameters()
    ...
    def get_full_edge_index(self, num_nodes: int):
        adj = torch.ones((num_nodes, num_nodes))  # [N, N]
        edge_index, _ = dense_to_sparse(adj)
        return edge_index  # [2, num_edges]       
        
        
    def forward(self, batch_x: Tensor, explanation: bool) -> Tensor:
        """GDN forward
        ...
        out, (index, alpha), self.alpha_raw = self.gnn(x, edge_index_repeat, embeddings_repeat)
        ...
        att_matrix_flat, att_matrix = extract_dependency_matrix(index, alpha, B, N, self.topk, device=self.device)
        att_recon = self.attn_ae(att_matrix_flat)
        
        plugin_loss = self.loss_fn_attn(att_recon, att_matrix_flat)
        att_score = torch.abs((att_recon-att_matrix_flat))

        if explanation:
            att_recon = att_recon.reshape(B, N, N)
            torch.save(att_matrix, 'dependency_matrix.pt')
            torch.save(att_recon, 'dependency_recon.pt')
        
        return out.view(B, N), plugin_loss, att_score

    
    def total_loss(self, pred, label, plugin_loss, lambda_attn):
        pred_loss = self.loss_fun(pred, label)
        return pred_loss + lambda_attn * plugin_loss